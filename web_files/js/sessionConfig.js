...
var sessionConfig = {
    ...
    setup: function () {
        ...
        sessionConfig.setupDamageType();
        sessionConfig.setupDamageScale();
        sessionConfig.setupDamageScaleSlider();
        sessionConfig.onChangeToDamageType();
        sessionConfig.setupMechanicalFailures();
        sessionConfig.onMechanicalFailuresChanged();
        sessionConfig.setupMinPlayerStrengthSlider();
        sessionConfig.addOnChangeToVehicleSelect();
        sessionConfig.setupRealWeatherToggles();
        sessionConfig.setupMandatoryPitstopsToggles();
        ...
        sessionConfig.setupPitSpeedLimitSlider();
    ...
    setupPitSpeedLimitSlider: function () {
        var gameSessions = sessionConfig.config.gameSessions;
        $.map(gameSessions, function (gameSession) {
            $("#" + gameSession + "PitSpeedLimitSlider").slider({
                value: $("#" + gameSession + "PitSpeedLimit").val(),
                min: 0,
                max: 200,
                step: 5,
                slide: function (event, ui) {
                    newVal = ui.value;
                    oldVal = parseInt( $("#" + gameSession + "PitSpeedLimit").val() );
                    ...
                    $("#" + gameSession + "PitSpeedLimit").val(newVal);
                    $("#" + gameSession + "PitSpeedLimitSlider").slider("value", newVal);
                }
            });
            $("#" + gameSession + "PitSpeedLimit").val($("#" + gameSession + "PitSpeedLimitSlider").slider("value"));
        });
    },
    ...
    setupMandatoryPitstopsToggles: function() {
        var gameSessions = sessionConfig.config.gameSessions;
        var sessionNames = sessionConfig.config.sessionNames;
        ...
        $.map(gameSessions, function (gameSession) {
            $.map(sessionNames, function (session) {
                var name = gameSession + session;
                $("#" + name + "MandatoryPitStops").on("change", function () {
                    sessionConfig.toggleMandatoryPitStopRules(name);
                });
            });
        });
    },

    toggleMandatoryPitStopRules: function( name ) {
        mandatoryPitStopRulesDisable = $("#" + name + "MandatoryPitStops").val() == 0;
        $("#" + name + "MandatoryPitStopsMinTyres").attr('disabled', mandatoryPitStopRulesDisable);
    },
    ...
    setupDamageScale: function()
    {
        var gameSessions = sessionConfig.config.gameSessions;

        $.map(gameSessions, function (gameSession) {
            $("#" + gameSession + "DamageType").on("change", function () {
                sessionConfig.onChangeToDamageType();
            });
        });
    },

    setupDamageScaleSlider: function () {

    },

    setupDamageType: function()
    {
        var gameSessions = sessionConfig.config.gameSessions;
    ...
    onChangeToDamageType: function() {
        var gameSessions = sessionConfig.config.gameSessions;
        $.map(gameSessions, function (gameSession) {
            var damageTypeVal = parseInt( $("#" + gameSession + "DamageType").val() );
            if (damageTypeVal < 2)
            {
                $("#" + gameSession + "DamageScale").attr('disabled', true);
            }
            else
            {
                $("#" + gameSession + "DamageScale").removeAttr('disabled');
            }
        });
    },

    setupMechanicalFailures: function() {

    },

    onMechanicalFailuresChanged: function() {
        ...
        $.map(gameSessions, function (gameSession) {
            $("input[name=" + gameSession + "LengthType]").on("change", function () {
                ...
                if ( $(this).is(':checked') && $(this).attr('id') == gameSession + "RadioMins" ) {
                    ...
                    $( "#" + gameSession + "RaceExtraLap").removeAttr("disabled");
                } else {
                    $( "#" + gameSession + "RaceMandatoryPitStops").removeAttr("disabled");
                    $( "#" + gameSession + "RaceExtraLap").attr("disabled", true);
    ...
    */
    saveAttributes: function (sessionToModify) {
        ...
        */
        ...
        autoAdvanceSession = $("input[name=" + sessionToModify + "AutoAdvanceSession]:checked").val();
        apiUrl = apiUrl + "session_AutoAdvanceSession=" + autoAdvanceSession + ampersand;
        ...
        raceExtraLap = $("#" + sessionToModify + "RaceExtraLap").val();    
        apiUrl = apiUrl + "session_RaceExtraLap=" + raceExtraLap + ampersand;
        ...
        mandatoryPitStopsDisabled = mandatoryPitStops == 0;
        mandatoryPitStopsMinTyres = mandatoryPitStopsDisabled ? 0 : $("#" + sessionToModify + "RaceMandatoryPitStopsMinTyres").val();
        apiUrl = apiUrl + "session_RaceMandatoryPitStopsMinTyres=" + mandatoryPitStopsMinTyres + ampersand;

        disablePitstopRefuelling = $("#" + sessionToModify + "DisablePitstopRefuelling").val();
        apiUrl = apiUrl + "session_DisablePitstopRefuelling=" + disablePitstopRefuelling + ampersand;
        ...
        damageScale = $("#" + sessionToModify + "DamageScale").val();
        apiUrl = apiUrl + "session_DamageScale=" + damageScale + ampersand;

        randomFailures = $("#" + sessionToModify + "DamageRandomFailures").val();
        apiUrl = apiUrl + "session_DamageRandomFailures=" + randomFailures + ampersand;

        fuelUsage = $("#" + sessionToModify + "FuelUsage").val();
        ...
        pitSpeedLimit = $("#" + sessionToModify + "PitSpeedLimit").val();
        iPitSpeedLimit = parseInt( pitSpeedLimit );
        if (iPitSpeedLimit < 40) { iPitSpeedLimit = 0; }
        apiUrl = apiUrl + "session_PitSpeedLimit=" + iPitSpeedLimit + ampersand;
        ...
        disableFullCourseYellows = $("input[name=" + sessionToModify + "FullCourseYellows]:checked").val();
        apiUrl = apiUrl + "session_FullCourseYellows=" + disableFullCourseYellows + ampersand;
    ...
    */
    commonPanelAttributes: function (sessionToModify, sessionName, ampersand) {
        ...
        else 
        {
            ...
            scheduledFullCourseYellow = $("#" + sessionToModify + sessionName + "ScheduledFullCourseYellow").val()
            apiUrl = apiUrl + "session_" + attrNamePrefix + "ScheduledFullCourseYellow=" + scheduledFullCourseYellow + ampersand;
        }

        if (allowPrivateSession)
        {
            privateSession = $("#" + sessionToModify + sessionName + "PrivateSession").prop("checked") ? 1 : 0;
            if (privateSession) {
                apiUrl = apiUrl + "session_" + attrNamePrefix + "PrivateSession=" + privateSession + ampersand;
            }
            else
            {
        ...
        liveTrackPreset = $("#" + sessionToModify + sessionName + "LiveTrackPreset").val()
        apiUrl = apiUrl + "session_" + attrNamePrefix + "LiveTrackPreset=" + liveTrackPreset + ampersand;
